...
namespace UnityEngine.LowLevel.PlayerLoopUtils
{
    public static class PlayerLoopUtils
    {
        ...
        public static PlayerLoopSystem? FindSubSystem<T>(ref PlayerLoopSystem loop)
        {
            if (loop.subSystemList == null) return null;

            for (int i = 0; i < loop.subSystemList.Length; ++i)
            {
                if (loop.subSystemList[i].type == typeof(T))
                {
                    return loop.subSystemList[i];
                }

                var found = FindSubSystem<T>(ref loop.subSystemList[i]);
                if (found.HasValue)
                {
                    return found.Value;
                }
